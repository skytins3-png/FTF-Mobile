(()=>{
  ...
  const AVAILABLE_SCENES={1:91,2:26,8:26};
  ...
  window.showComic=async function(n,cut){
    ...
    // Scene 77 is the approved raster cinematic panel; scene 91 is raster artwork embedded in SVG; the rest remain vector scenes.
    const ext=(n===1&&sceneNumber===77)?'jpg':'svg';
    const src=`${BASE}FTF-${String(n).padStart(2,'0')}/${String(sceneNumber).padStart(2,'0')}.${ext}?v=115`;
